...
class Flushbar<T extends Object> extends StatefulWidget {
  ...
  late FlushbarStatusCallback onStatusChanged;
  ...
  Future<T?> dismiss([T? result]) async {
    ...
    final mFlushbarRoute = _flushbarRoute;
    if (mFlushbarRoute == null) {
      return null;
    }

    if (mFlushbarRoute.isCurrent) {
      mFlushbarRoute.navigator?.pop(result);
      return mFlushbarRoute.completed;
    } else if (mFlushbarRoute.isActive) {
      ...
      mFlushbarRoute.navigator?.removeRoute(mFlushbarRoute);
    }
...
class _FlushbarState<K extends Object> extends State<Flushbar>
  ...
  void dispose() {
    ...
    if (_progressListener != null) {
      widget.progressIndicatorController
          ?.removeListener(_progressListener!.call());
    }
  ...
  void _configureLeftBarFuture() {
    SchedulerBinding.instance.addPostFrameCallback(
      (_) {
        ...
        if (keyContext != null) {
          _boxHeightCompleter.complete(keyContext.size);
        }
      },
    );
  }
  ...
  Widget _generateInputFlushbar() {
    return Container(
      ...
      child: Padding(
        ...
        child: FocusScope(
          child: widget.userInputForm ?? const SizedBox(),
          node: _focusNode,
          autofocus: true,
        ),
      ),
  ...
  Widget _getCloseIcon() {
    if (widget.closeIcon != null &&
        widget.closeIcon is Icon &&
        widget.shouldIconPulse) {
      return FadeTransition(
        opacity: _fadeAnimation,
