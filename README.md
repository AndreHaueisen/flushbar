...
leftBarIndicatorBuilder | If not null, will be used to build and show a left vertical bar to better indicate the humor of the notification. It is not possible to use it with a [Form] and I do not recommend using it with [LinearProgressIndicator]. Type: `Widget LeftBarIndicatorBuilder(BuildContext context, Size size)`
