...
~~borderRadius~~ | Deprecated, use `decoration`. Adds a radius to all corners of Flushbar. Best combined with [aroundPadding]. I do not recommend using it with [showProgressIndicator] or [leftBarIndicatorColor]
~~backgroundColor~~ | Deprecated, use `decoration`! Flushbar background color. Will be ignored if [backgroundGradient] is not null.
...
~~boxShadows~~ | Deprecated, use `decoration`! The shadows generated by Flushbar. Leave it null if you don't want a shadow. You can use more than one if you feel the need. Check [this example](https://github.com/flutter/flutter/blob/master/packages/flutter/lib/src/material/shadows.dart)
~~backgroundGradient~~ | Deprecated, use `decoration`! Flushbar background gradient. Makes [backgroundColor] be ignored.
